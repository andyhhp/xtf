...
# define PKR_WD_VAL 0xaaaaaaaa
# define PKR_AD_VAL 0x55555555
#else
# define PKR_WD_VAL 0x80000000
# define PKR_AD_VAL 0x40000000
#endif
...
static pg_ctrl_t get_current_pg_ctrl(void)
{
    ...
    if ( cpu_has_pku )
    {
        ...
        ctrl.pkru_wd = pkru & PKR_WD_VAL;
        ctrl.pkru_ad = pkru & PKR_AD_VAL;
    ...
    if ( cpu_has_pks )
    {
        uint32_t pkrs = rdmsr(MSR_PKRS);

        ctrl.pkrs_wd = pkrs & PKR_WD_VAL;
        ctrl.pkrs_ad = pkrs & PKR_AD_VAL;
    }
...
static void __printf(1, 2) fail(const char *fmt, ...)
{
    ...
    if ( !under_test.pte_printed )
    {
        ...
        printk("  CTRL:%s%s%s%s%s%s%s%s%s%s%s\n",
               ctrl.pkrs_ad ? " PKSAD" : "",
               ctrl.pkrs_wd ? " PKSWD" : "",
               ctrl.pks     ? " PKS"   : "",
               ctrl.pkru_ad ? " PKUAD" : "",
...

{
    ...
    {
        if ( CONFIG_PAGING_LEVELS == 4 && /* Check Protection Key */
             m->leaf && (((pte >> 59) & 0xf) == 0xf) )
        {
            if ( pte & PFEC_SYM(U) ) /* User mapping */
            {
                if ( ctrl.pke &&
                     (ctrl.pkru_ad ||
                      ((walk & PFEC_SYM(W)) && ctrl.pkru_wd &&
                       ((walk & PFEC_SYM(U)) || ctrl.wp))) )
                    return fault | PFEC_SYM(K);
            }
            else /* Supervisor mapping */
            {
                if ( ctrl.pks &&
                     (ctrl.pkrs_ad ||
                      ((walk & PFEC_SYM(W)) && ctrl.pkrs_wd &&
                       ((walk & PFEC_SYM(U)) || ctrl.wp))) )
                    return fault | PFEC_SYM(K);
            }
        }
...
static void foreach_pg_ctrl_setting(unsigned int level, bool super)
{
    ...
    missing.pks =  missing.pkrs_wd = missing.pkrs_ad = !cpu_has_pks;

    uint64_t efer     = rdmsr(MSR_EFER);
    uint32_t pkrs     = cpu_has_pks && rdmsr(MSR_PKRS);
    ...
    for ( ctrl = 0; ctrl < max; ++ctrl )
    {
        ...
        unsigned long ncr4 =
            ((cr4 & ~(X86_CR4_SMEP | X86_CR4_SMAP | X86_CR4_PKE | X86_CR4_PKS)) |
             (c.smep ? X86_CR4_SMEP : 0) |
             (c.smap ? X86_CR4_SMAP : 0) |
             (cpu_has_pku ? X86_CR4_PKE : 0) | /* Always set PKE to edit PKRU */
             (c.pks  ? X86_CR4_PKS : 0));
        if ( ncr4 != cr4 )
            write_cr4(cr4 = ncr4);
        ...
        if ( cr4 & X86_CR4_PKE )
        {
            wrpkru((c.pkru_wd ? PKR_WD_VAL : 0) |
                   (c.pkru_ad ? PKR_AD_VAL : 0));
            if ( !c.pke )
                write_cr4(cr4 = (cr4 & ~X86_CR4_PKE));
        }

        uint32_t npkrs = ((c.pkrs_wd ? PKR_WD_VAL : 0) |
                          (c.pkrs_ad ? PKR_AD_VAL : 0));
        if ( npkrs != pkrs )
            wrmsr(MSR_PKRS, pkrs = npkrs);
    ...
    write_cr4(cr4 & ~(X86_CR4_SMEP | X86_CR4_SMAP | X86_CR4_PKE | X86_CR4_PKS));
    if ( cpu_has_pks )
        wrmsr(MSR_PKRS, 0);
}
