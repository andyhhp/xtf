...
ifneq ($(VARY-CFG),)
TEST-CFGS := $(foreach env,$(TEST-ENVS),$(foreach vary,$(VARY-CFG),test-$(SUBARCH)-$(env)-$(NAME)~$(vary).cfg))
else
TEST-CFGS := $(foreach env,$(TEST-ENVS),test-$(SUBARCH)-$(env)-$(NAME).cfg)
endif

.PHONY: build
build: $(foreach env,$(TEST-ENVS),test-$(SUBARCH)-$(env)-$(NAME)) $(TEST-CFGS)
build: info.json

info.json: $(ROOT)/build/mkinfo.py FORCE
	@$(PYTHON) $< $@.tmp "$(NAME)" "$(CATEGORY)" "$(SUBARCH)" "$(TEST-ENVS)" "$(VARY-CFG)"
...
# If any subarchitecture/environment needs a special compilation/linking recipe
# instead of the default one, a custom recipe called build-$(SUBARCH) or
# build-$(env) e.g. build-arm or build-hvm64 should be created in
# $(ROOT)/build/$(ARCH)/arch-common.mk

test-$(SUBARCH)-$(1)-$(NAME): $$(DEPS-$(1)) $$(link-$(1))
ifdef build-$(SUBARCH)
	@# Sub-architecture specific compilation recipe
	$(call build-$(SUBARCH),$(1))
else ifdef build-$(1)
...
test-$(SUBARCH)-$(1)-$(NAME).cfg: $$(cfg-default-deps)
	$(PYTHON) $$< $$@.tmp "$$(cfg-$(1))" "$(VCPUS)" "$(TEST-EXTRA-CFG)" ""
	@$(call move-if-changed,$$@.tmp,$$@)

test-$(SUBARCH)-$(1)-$(NAME)~%.cfg: $$(cfg-default-deps) %.cfg.in
	$(PYTHON) $$< $$@.tmp "$$(cfg-$(1))" "$(VCPUS)" "$(TEST-EXTRA-CFG)" "$$*.cfg.in"
	@$(call move-if-changed,$$@.tmp,$$@)

test-$(SUBARCH)-$(1)-$(NAME)~%.cfg: $$(cfg-default-deps) $(ROOT)/config/%.cfg.in
...
install-$(1): test-$(SUBARCH)-$(1)-$(NAME)
	@$(INSTALL_DIR) $(DESTDIR)$(xtftestdir)/$(NAME)
	$(INSTALL_PROGRAM) $$< $(DESTDIR)$(xtftestdir)/$(NAME)

install-$(1).cfg: $(filter test-$(SUBARCH)-$(1)-%,$(TEST-CFGS))
	@$(INSTALL_DIR) $(DESTDIR)$(xtftestdir)/$(NAME)
...
clean:
	...
	rm -f $(foreach env,$(TEST-ENVS),test-$(SUBARCH)-$(env)-$(NAME) test-$(SUBARCH)-$(env)-$(NAME)*.cfg)
